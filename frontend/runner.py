"""后台线程运行器，用于执行 AShareAgentsGraph 分析流水线。"""
...
def _run(ticker: str, trade_date: str, config: dict, tracker: ProgressTracker) -> None:
    """在当前线程中执行完整分析流水线。"""
    from frontend.stats_handler import StatsCallbackHandler
    from AShareAgents.workflows.trading_graph import AShareAgentsGraph

    stats = StatsCallbackHandler()

    graph = AShareAgentsGraph(
        debug=True,
        config=config,
        callbacks=[stats],
    )
