...
def render_progress(tracker: ProgressTracker) -> None:
    ...
    st.markdown(
        '<div style="margin:0.5rem 0 0.3rem; font-size:0.85rem; color:#888;">分析师</div>',
        unsafe_allow_html=True,
    )
    ...
    st.markdown(
        '<div style="margin:0.8rem 0 0.3rem; font-size:0.85rem; color:#888;">流水线</div>',
        unsafe_allow_html=True,
    )
    ...
    if completed_reports:
        st.markdown(
            ...
            f"报告 ({len(completed_reports)})</div>",
            unsafe_allow_html=True,
        )
